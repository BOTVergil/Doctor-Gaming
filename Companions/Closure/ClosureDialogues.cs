...
namespace DocterMain.Companions.Docter
{
    ...
    public class ClosureDialogues : CompanionDialogueContainer //Must be assigned on the companion base file, setting it as the value of "GetDialogueContainer" overrideable method.
    {
        public override string GreetMessages(Companion companion) //Messages for when you just met the companion.
        {
            return "We meet again, Doctor.";
        }
                ...
                case RequestContext.HasRequest:
                    {
                        ...
                        Mes.Add("Hey Doc! Can you help me with something? I need you to [objective]. I'm not really used to ask for help but.. Can you help me with this?");
                        return Mes[Main.rand.Next(Mes.Count)];
                    }
                case RequestContext.Completed:
                    {
                        List<string> Mes = new List<string>();
                        Mes.Add("You're the best, Doc!");
        ...
        public override void ManageOtherTopicsDialogue(Companion companion, MessageDialogue dialogue) //Allow you to add new dialogues to the other topics dialogue. There's one for lobby dialogues too.
        {
            ...
            dialogue.AddOptionAtTop("I want to do some trade.", (Action)tradewithClosure);
        }
        ...
        private void tradewithClosure()
	    {
		MultiStepDialogue val = new MultiStepDialogue(new string[1] { "If you have anything you want to trade, just say it, Doc?" }, (Companion)null);
		val.AddOption("Yes,I want to buy some bullets.", (Action)GiveBullets);
        val.AddOption("Yes,I want to buy some medicine.", (Action)GiveMedicine);
		val.AddOption("Nevermind.", (Action)ChangeMind);
		((MessageBase)val).RunDialogue();
	    }
        private void GiveMedicine()
	    {
		MessageDialogue md = new MessageDialogue("", (DialogueOption[])null);
		if (MainMod.GetLocalPlayer.HasItem(72))
		{
			md.ChangeMessage("Do you want these medications?", (Companion)null);
			md.AddOption("Yes.", (Action)Closuregivemedicine);
		}
        else
		{
			md.ChangeMessage("Where is the money?", (Companion)null);
			md.AddOption("Sorry,i forget it somewhere", (Action)ForgetIt);
		}
		((MessageBase)md).RunDialogue();
	    }
		...
		if (MainMod.GetLocalPlayer.HasItem(72))
		{
			md.ChangeMessage("Are you sure you want to obtain these ammunition?", (Companion)null);
			md.AddOption("Yes.", (Action)Closuregivebullet);
		}
        else
		{
			md.ChangeMessage("Where is the money?", (Companion)null);
        ...
        private void Closuregivebullet()
	    {
		MessageDialogue val = new MessageDialogue("This is your ammunition, but it's a bit scarce.", (DialogueOption[])null);
		GivebulletRandomLootToPlayer(Main.rand.Next(65, 150));
		val.AddOption("Thank you.", (Action)EndDialogue);
		((MessageBase)val).RunDialogue();
	    }
        private void Closuregivemedicine()
	    {
		MessageDialogue val = new MessageDialogue("This is your medication, but it's a bit rare.", (DialogueOption[])null);
		GivemedicineRandomLootToPlayer(Main.rand.Next(10, 20));
		val.AddOption("Thank you.", (Action)EndDialogue);
		((MessageBase)val).RunDialogue();
	    }
        private void GivebulletRandomLootToPlayer(int loot)
	    {
		for (int i = 0; i < 50; i++)
		{
			if (MainMod.GetLocalPlayer.inventory[i].type == 72)
			{
				Item obj = MainMod.GetLocalPlayer.inventory[i];
				obj.stack--;
				if (MainMod.GetLocalPlayer.inventory[i].stack <= 0)
				{
					MainMod.GetLocalPlayer.inventory[i].TurnToAir(false);
				}
				break;
			}
		}
		Item.NewItem((IEntitySource)new EntitySource_Misc(""), ((Entity)MainMod.GetLocalPlayer).Center, Vector2.One, 97, loot, false, 0, false, false);
	    }
        private void GivemedicineRandomLootToPlayer(int loot)
	    {
		for (int i = 0; i < 50; i++)
		{
			if (MainMod.GetLocalPlayer.inventory[i].type == 72)
		...
		Item.NewItem((IEntitySource)new EntitySource_Misc(""), ((Entity)MainMod.GetLocalPlayer).Center, Vector2.One, 188, loot, false, 0, false, false);
	    }
        private void ChangeMind()
	    {
		Dialogue.TalkAboutOtherTopicsDialogue("Ok,so what else do you want to talk about?");
	    }
	    private void ForgetIt()
	    {
		Dialogue.TalkAboutOtherTopicsDialogue("....");
	    }
        private void EndDialogue()
	    {
		Dialogue.TalkAboutOtherTopicsDialogue("Bye,and thank you for the treat!");
	    }
    }
}