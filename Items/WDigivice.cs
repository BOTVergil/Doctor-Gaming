...
{
    public class WDigivice : ModItem
    {
        ...
        public override bool CanUseItem(Player player)
        {
            if (UseItem(player))
            {
                ...
                Companion tg = 
                terraguardians.WorldMod.SpawnCompanionNPC(player.Bottom, DigimonContainer.W, Mod.Name); //Spawns Docter as a companion npc, on the position of the player (Companion spawn orientation is based on the bottom position, so aim for the feet)
            ...
            Main.NewText("There is already a W.");
            return false;
        }
