...
{
    public class ExusiaiMedalA : ModItem
    {
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            {
                Main.NewText("The elite level of Exusiai has been increased to 1.");
                Item.SetDefaults(0);
                EliteSystem.NengEliteA = true;
                return true;
            }
            if (EliteSystem.NengEliteA && !EliteSystem.NengEliteB)
            {
                Main.NewText("Exusiai level has reached 1.");
                return false;
            }
            if (EliteSystem.NengEliteA && EliteSystem.NengEliteB)
            {
                Main.NewText("Exusiai level has reached 2.");
                return false;
            }
            return false;
        }
